 ...
 * 红色节点的子节点必须为黑
 * 节点颜色非红即黑
 * 从一个节点到一个null引用的每条路径包含相同数目的黑色节点
 *
 * 节点属性：节点值、父节点、左子节点、右子节点、颜色
 *
 * 红黑树包含：黑色的根节点
 *
 * 左旋转：新节点是父节点的右子节点且父节点是祖父节点的右子节点
 *             13                               17
 *      *     /  \                             /  \
 *      *   nul  17                          13    27
 *      *       / \      --(左旋)-.          / \    / \
 ...
 *      *         / \
 *      *      nul  nul
 *      27为新节点，13为方法输入的祖父节点,以父节点为轴
 *      先确定祖父节点旋转之后的右子节点（双向，父节点为谁，字节点为谁）
 *      父节点旋转之后的父节点
 *      父节点旋转之后与祖父节点的关系
 *      leftRotate(Node g){
 *          p=g.right;
 ...
 *      }
 *
 *      插入元素，先按二叉搜索数插入元素，插入节点为红色，再修正
 *      if(父节点为空或者父节点为黑)直接插入，根节点设置黑
 *      if(父节点为红，叔父节点为红)插入，变色，父节点层设置黑，祖父节点为红，再对祖父节点进行递归判断
 *      if(父节点为红，叔父为黑)，进行旋转
 *
 */

public class RBTreeNote<T extends Comparable<T>> {
